var sq1_scrambler = (function() {
var seqlen=40;
 
function scramble(){
    var i,j,ls,n=0,f;
		var seq = [];
        var posit = new Array(0,0,1,2,2,3,4,4,5,6,6,7,8,9,9,10,11,11,12,13,13,14,15,15);
        ls=-1;
        seq[n]=new Array();
        f=0;
        for(i=0; i<seqlen; i++){
            ...
            }while( (f>1 && j>=-6 && j<0) || domove(j, posit) );
            if(j>0) ls=1;
            else if(j<0) ls=2;
            else { ls=0; f++; }
            seq[n][i]=j;
...
return {
	scramble: function() {
		return scramblestring(scramble()[0]);
	}
}
}());
